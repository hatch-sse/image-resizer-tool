...
function setSkinUI(){
  ...
  if (sseToggle) {
    sseToggle.checked = isSse;
    sseToggle.setAttribute("aria-label", isSse ? "Switch off SSE.com skin" : "Switch on SSE.com skin");
  }
  ...
  if (websiteTab) {
    const sseLabel = websiteTab.dataset.sseLabel || "News & Views";
    const defaultLabel = websiteTab.dataset.defaultLabel || "Full website";
    websiteTab.textContent = isSse ? sseLabel : defaultLabel;
  }

  const articleTab = document.querySelector('.presetTab[data-tab="article"]');
  if (articleTab) {
    articleTab.style.display = isSse ? "none" : "";
  }

  const ssePresetWrapper = document.querySelector(".sse-preset-wrapper");
  const ssenPresetWrapper = document.querySelector(".ssen-preset-wrapper");

  if (ssePresetWrapper) ssePresetWrapper.style.display = isSse ? "" : "none";
  if (ssenPresetWrapper) ssenPresetWrapper.style.display = isSse ? "none" : "";

  const activeArticleTab = document.querySelector('.presetTab.active[data-tab="article"]');
  if (isSse && activeArticleTab) {
    activateTab("website");
  }
}
...
if (sseToggle) {
  sseToggle.addEventListener("change", () => {
    document.body.classList.toggle("sse", sseToggle.checked);

    if (document.body.classList.contains("sse")) {
...
if (brandLogo) {
  brandLogo.addEventListener("click", () => {
    document.body.classList.toggle("mac9");
    setSkinUI();
  });
}
